...
int main(int argc, char *argv[]){
  ...
  while(!main_done){
	...
	if(sdl_access->get_v_bar().is_scrolling()){//if the vertical scroll bar is in "scroll mode"
		   //do a mini loop until the left mouse button is released
		scrolling_mini_loop(big_event,/*sdl_helper,*/b_manager,'v');
		sdl_access->get_v_bar().scroll_mode_change(false);//stop v scroll bar mode
		SDL_FlushEvents(0,1000); //is this necessary?


	} else if(sdl_access->get_h_bar().is_scrolling()){//if the horizontal scroll bar is in "scroll mode"
		     //do a mini loop until the left mouse button is released
		scrolling_mini_loop(big_event,/*sdl_helper,*/b_manager,'h');
		sdl_access->get_h_bar().scroll_mode_change(false);//stop h scroll bar mode
	...
	switch(big_event.type){ //switch controlled by the 'type' of input given, like the mouse moving
		...
		case SDL_WINDOWEVENT:
			...
			if(big_event.window.event == SDL_WINDOWEVENT_SIZE_CHANGED){
				...
				sdl_access->window_update(big_event.window.data1,big_event.window.data2);
	...
	sdl_access->draw();
	sdl_access->present();  //and all positions have been calculated
